...
- **文章标题**: Living Room Feng Shui for the Fire Month: Why the Sofa Stops Working in June and What to Move Instead
- **字数**: 4,254 words
- **去AI化评分**: 95/100 (first pass 79/100 → patched `actually` → `always` and `tapestry` → `framed textile` → 95/100; only remaining note is high word count, which is the article's natural length)
- **发布的 URL**: https://oriental-destiny.com/fate-2026-06-17.html (HTTP 200 verified)

**Sequence in the seasonal thread:**
- 06-10: Center of home (Earth sector)
- 06-11: Bing Day Master (Yang Fire chart primer)
- 06-12: Xia Zhi (Summer Solstice)
- 06-13: Ming Tang in Summer (Entryway)
- 06-14: Bedroom Fire Month
- 06-15: Home Office Fire Month
- 06-16: Kitchen Fire Month
- **06-17: Living Room Fire Month** ← today, completes the major-room walk

**Push notes:** the remote had reverted to HTTPS again — switched back to SSH (`git@github.com:qzw-alt/oriental-destiny.git`), pushed cleanly. Commit `91cd380`.
