...
```
RuntimeError: [ Cron budget exhausted — research phase only. No article written, no commit, no push. ]

## Cron Run 2026-06-16 Summary

### Pre-flight (clean)
- Working tree clean, no pending recovery needed
- Last commit `c8bffec` already on `origin/master` (2026-06-14 Akeso gumokimab, shipped cleanly yesterday)
- SSH remote `git@github.com:qzw-alt/chinahospitalsguide.git` is in place
- Branch: `master`

### Research state
Searched Bing News (4 queries), ChinaDaily China/Business/Health/Asia sections (15+ article titles checked), Akeso press archive, Manila Times PR Newswire (06-13 → 06-16 range). All Yahoo Finance / BusinessWire / ContractPharma endpoints blocked (Cloudflare / 23-byte responses). Found **two candidates**, neither shippable today due to budget exhaustion.

### Candidate 1 — CarsGen at EHA 2026 (strongest, but blocked by rate-limit)
- Allogeneic CAR-T products CT0596 + CT1190B presented at EHA 2026 (June 12-15, Milan)
- Source via wvgazettemail.com PR mirror — body returned `Too Many Requests`
- Next-run action: retry via `https://www.manilatimes.net/2026/06/{12-15}/tmt-newswire/pr-newswire/carsgen-...` mirror, or fetch directly from CarsGen's IR page (carsgen.com)
- Why shippable: allogeneic CAR-T (off-the-shelf, no need for patient-specific manufacturing) is a step-change for medical-tourism economics; complements the 06-09 Oricell GPC3 (autologous solid-tumor) and 06-10 Antengene ATG-201 (autoimmune bispecific) coverage

### Candidate 2 — HK Lung Foundation COPD/asthma survey (June 15, weak angle)
- 67.6% of HK residents lack adequate COPD/asthma knowledge; 25.9% of HK adults 60+ have COPD; 330-400k asthma patients
- Source: ChinaDaily Asia article 635004
- Marginal: HK public-health policy, not directly medical-tourism relevant

### Akeso June 15 (rejected)
- HER3 ADC AK138D1 + ivonescimab first-patient-enrolled (Phase Ib/II breast cancer)
- Two consecutive Akeso articles (06-14 gumokimab + 06-15 AK138D1) would read as channel-overload; first-patient-enrolled is lower news value than approval or Phase III data

### Pending-file handoff
Writing the pending handoff so the next cron run (2026-06-17) can pick this up without re-researching.]<]minimax[>[<tool_call>
]<]minimax[>[<invoke name="terminal">]<]minimax[>[<command>mkdir -p /home/ubuntu/.hermes/skills/research/content-research-writer-cn/references && cat > /home/ubuntu/.hermes/skills/research/content-research-writer-cn/references/pending-2026-06-16-carsgen-eha-allogeneic-cart.md <<'EOF'
# Pending handoff — 2026-06-16 cron run (cron cap hit during research)

## Status
- Cron iteration cap reached during research phase
- No article written, no commit, no push
- Pre-flight: working tree clean, last commit `c8bffec` (06-14 Akeso gumokimab) already on origin/master
- SSH remote confirmed in place (`git@github.com:qzw-alt/chinahospitalsguide.git`)
- Branch: master

## Recommended topic for 2026-06-17 — CarsGen at EHA 2026 (allogeneic CAR-T)
**Hot topic:** CarsGen presented Phase 1 / preclinical data on its allogeneic (off-the-shelf) CAR-T cell products CT0596 and CT1190B at the European Hematology Association (EHA) 2026 Congress (Milan, June 12-15, 2026).

### Why shippable
- **Allogeneic CAR-T** is the next step-change for medical tourism: no need for patient-specific cell collection/manufacturing, lower cost, faster turnaround, scale-up compatible with international patient flow
- Complements recent coverage: 06-09 Oricell Ori-C101 (autologous solid-tumor GPC3 CAR-T, HCC), 06-10 Antengene ATG-201 (CD19×CD3 bispecific TCE for autoimmune), 06-14 Akeso gumokimab (anti-IL-17 mAb for psoriasis)
- EHA 2026 = international clinical congress = high credibility + Chinese biotech co-authorship angle
- The "off-the-shelf" economics angle aligns with medical-tourism thesis: international patients who cannot afford US/EU autologous CAR-T ($500K+) or who cannot wait 4-6 weeks for manufacturing

### Research that already happened (today)
- Bing News hit `https://www.wvgazettemail.com/online_features/press_releases/carsgen-presents-allogeneic-car-t-cell-products-ct0596-and-ct1190b-at-eha-2026/article_244c6c22-e659-53f4-ba17-cac65d5ae120.html` but body returned `Too Many Requests` (rate-limited)
- Manila Times PR Newswire feed scanned 06-12 to 06-16 — no CarsGen entry found

### Fetch recipe for next run
1. Try the manilatimes.net mirror pattern (verified working for Antengene 06-10 and Oricell 06-09):
   ```
   curl -A "Mozilla/5.0 ..." "https://www.manilatimes.net/2026/06/{12,13,14,15}/tmt-newswire/pr-newswire/carsgen-eha-2026" | grep -oE 'href="[^"]+"' | head
   ```
   Or search Bing: `CarsGen EHA 2026 allogeneic CT0596 CT1190B site:manilatimes.net`
2. If that fails, fetch directly from CarsGen IR:
   ```
   curl -A "Mozilla/5.0 ..." "https://www.carsgen.com/en/Media/News" | grep -oE 'href="[^"]+"' | grep -i '2026'
   ```
3. If both blocked, use CrossRef for the EHA abstract (EHA abstracts get DOIs):
   ```
   curl "https://api.crossref.org/works?query.bibliographic=CarsGen+CT0596+EHA+2026" | python3 -c "..." (blocked — use bundled extract.py instead)
   ```

### Key facts to extract (any source)
- Drug names: CT0596 (BCMA-targeted allogeneic CAR-T for multiple myeloma? — verify), CT1190B (CD19-targeted allogeneic CAR-T for B-cell lymphoma? — verify)
- Indications
- Patient numbers (n=X)
- ORR / CR rate / safety profile
- Manufacturing approach (gene-edited TCR-knockout / HLA-typed? — confirm)
- China NMPA trial status (IND / Phase I / etc.)
- EHA session details (poster / oral / abstract number)

### Suggested article structure (9 sections, ~3,500 words)
1. Lead — what EHA 2026 showed, why allogeneic matters for international patients
2. The autologous CAR-T bottleneck — why off-the-shelf is the next frontier (medical-tourism economics: $500K US autologous vs projected <$200K allogeneic, 4-6 weeks vs <1 week)
3. CT0596 data — myeloma cohort, efficacy, safety
4. CT1190B data — lymphoma cohort, efficacy, safety
5. The allogeneic manufacturing problem — TCR-knockout, HLA editing, lymphodepletion considerations
6. China angle — CarsGen's Zhangjiang cell-therapy ecosystem, NMPA parallel trial path, Shanghai Ruijin / Tongji / Jiahui International as access hospitals
7. International patient pathway — comparison with US/EU autologous CAR-T access timelines and costs
8. Risks and unknowns — durability, GvHD risk, manufacturing scalability
9. CTA — link to hospitals, link to China Hospitals Guide CAR-T hub, link to 06-09 Oricell + 06-10 Antengene pieces

### Internal links to use
- /news/2026-06-09-oricell-gpc3-cart-hcc-nmpa-phase2-clearance.html (sister article: autologous solid-tumor CAR-T)
- /news/2026-06-10-antengene-atg-201-bispecific-autoimmune-pku.html (sister article: bispecific TCE autoimmune)
- /news/2026-05-09-car-t-therapy-cost-china-solid-tumor-breakthrough.html (CAR-T cost framing)
- /news/2026-06-11-china-medical-tourism-cutting-edge-cheap-bloomberg.html (medical-tourism economics anchor)

### External links to use
- EHA 2026 official site (ehaweb.org) for the abstract
- CarsGen official IR (carsgen.com/en/Media/News) for canonical press release
- 1-2 NMPA / clinicaltrials.gov references if available

### Em-dash target
- 17-23 per 1200 words (chinahospitalsguide baseline)
- Use clinical parentheticals for drug mechanism (gene-editing, HLA-typed, lymphodepletion), trial design (Phase 1 dose-escalation), patient population (R/R, ECOG 0-1)

### Banned-vocab awareness
- Avoid: pivotal, crucial, landscape, actually, leverage, navigate, showcase, testament
- "Pivotal" was a false-positive on the 06-14 article in clinical-prose context — use "registrational" or "first-in-class" for the Phase 3 / approval language
- "Navigate" — only use in clinical-prose logistics sentences; CTA copy should say "move through" or "work through"

### De-dup grep before writing (in news/ directory)
```bash
cd /home/ubuntu/.hermes/workspace/website/news && grep -lE "(CT0596|CT1190B|allogeneic.CAR.T|EHA.2026|CarsGen)" *.html
```
Expected return: 0 matches (none of these terms appear in existing articles — Antengene CD19×CD3 and Oricell GPC3 are different targets and modalities). If the grep returns matches, switch topic.

## Recovery recipe
1. Read this file
2. Fetch the CarsGen / EHA source via the recipe above (try manilatimes first, then carsgen.com IR)
3. Verify date is 2026-06-12 to 2026-06-15 (EHA 2026 window)
4. Run de-dup grep (above)
5. Write article `news/2026-06-17-carsgen-eha-allogeneic-cart-eha-2026.html` using the 9-section structure (date = cron run date, NOT press release date, since the press release date is co-terminal with the cron run)
6. Humanize to >60 (likely needs 3-5 banned-vocab patches first time)
7. Update sitemap.xml + news/index.html
8. Commit + push + sleep 180 + curl HTTP 200 verify
9. Write next pending note `pending-2026-06-17-carsgen-eha-allogeneic-cart-shipped.md` documenting the run

## Cron state at end of run
- Budget exhausted during research phase (~15 tool calls)
- No file modifications
- No commits
- No push
EOF
echo "pending file written"
wc -l /home/ubuntu/.hermes/skills/research/content-research-writer-cn/references/pending-2026-06-16-carsgen-eha-allogeneic-cart.md]<]minimax[>[</command>]<]minimax[>[</invoke>
]<]minimax[>[</tool_call>
```
