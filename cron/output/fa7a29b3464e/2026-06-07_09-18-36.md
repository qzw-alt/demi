...
**Git push authentication failure (historical — RESOLVED 2026-06-07):** the oriental-destiny.com cron runs on 2026-06-03, 2026-06-04, and 2026-06-06 all hit GitHub HTTPS auth failures (`fatal: could not read Username` / `Password authentication is not supported for Git operations`) because the remote URL had no embedded token and the cron-sandbox credential helper masked the PAT. The 2026-06-06 troubleshooting run switched the remote to SSH (`git remote set-url origin git@github.com:qzw-alt/oriental-destiny.git`) using the existing `~/.ssh/id_ed25519` key. The 2026-06-07 cron run confirmed the fix is durable: `git push origin main` succeeded cleanly with no auth gymnastics. See `references/push-credential-troubleshooting.md` for the full diagnosis and SSH-first recipe. **If the push ever fails again, first check `git remote -v` — the URL should be `git@github.com:...` (SSH), not `https://github.com/...`.** If it has reverted to HTTPS, repeat the SSH switch from the troubleshooting reference.
...
```
RuntimeError: ## Final Report — chinahospitalsguide.com Cron 2026-06-07

### Recovery action completed (06-06 article)
- Discovered `8a6209d` (2026-06-06 Pakistani CAR-T article) was committed locally on 2026-06-06 but **never pushed** — fourth consecutive cron run to fail on the same `fatal: could not read Username` GitHub auth issue.
- Applied the same fix that worked for oriental-destiny.com on 2026-06-07: **switched the remote from `https://github.com/qzw-alt/chinahospitalsguide.git` to `git@github.com:qzw-alt/chinahospitalsguide.git`** (SSH using `~/.ssh/id_ed25519`).
- Remote had diverged (2 GA4 tracking commits from another agent). Merged `origin/master` with local master, kept the 06-06 commit on top.
- **Push succeeded** (`94786a1..a00651a master -> master`).
- **Verified live**: `curl https://chinahospitalsguide.com/news/2026-06-06-pakistani-patient-cart-shanghai-jiahui-lymphoma.html` → **HTTP 200** (after 3 min).

### Today's research (06-07)
- **Hot topic selected**: Dr. Syed Mohammed Ghouse (Indian-origin urologist, Tongji Hospital Wuhan) performed a 90-minute robot-assisted ureteral reimplantation on a patient 3,000 km away in Hyderabad, India, on a Chinese-developed surgical robot over a Chinese 5G network (~200 ms latency). One of 26 live surgeries at the 10th IHPBA Congress, 5 of them cross-border (Brazil, Georgia, Greece, Uzbekistan, India).
- **Sources verified**:
  - China Daily 2026-05-20 (primary Chinese-side, `<meta name="publishdate" content="2026-06-20">` ... 2026-05-20)
  - Times of India 2026-05-24 (full narrative, 90 min, 3,000 km)
  - NDTV, The Hindu, News18, NewsBytes, Deccan Chronicle (corroborating)
- **Angle**: "China-developed surgical robotics + 5G exported for cross-border clinical service" — a hedge/complement to the inbound medical tourism narrative the site has been covering.

### Article drafted (NOT YET PUBLISHED)
- **File**: `/home/ubuntu/.hermes/workspace/website/news/2026-06-07-5g-remote-surgery-tongji-wuhan-india-china-medical-tech.html`
- **Length**: ~4,059 words (9 sections: The Case / How It Worked / Chinese Robots Compared / The 5G Network / Tongji's Role / Tele-Surgery Milestones Table / Implications for Medical Tourism / What to Watch / Conclusion)
- **Banner color**: deep blue `#1a3a5c → #2c5f8d → #4a90c2` (different from 06-06 teal, 06-05 blue, 06-03 navy)
- **External sources**: 10 links including China Daily, Times of India, NDTV, The Hindu, Cornerstone/Sentire press release, Kunwu Brazil release
- **Internal links**: 06-05 ivonescimab, 06-03 Hainan Boao, 06-06 Pakistani CAR-T, /services.html, /contact-new.html
- **Humanizer pass in progress**:
  - 1 "Actually" → "in Practice" (fixed)
  - Em-dash density was 11.3/1200; added ~6 clinical-aside em-dashes to push toward the 17-23/1200 site baseline (added parentheticals for: hardware comparison, the 10th IHPBA context, India-as-test-bed, scale argument, control logic, regulatory gap, etc.)

### NOT completed (budget exhausted mid-humanize-pass)
- Final em-dash density recheck via `em_dash_check.py`
- Sitemap entry insertion at top of `sitemap.xml`
- News card insertion at top of `news/index.html`
- `git checkout -b article-2026-06-07 origin/master` + commit + push
- Post-push HTTP 200 verification

### Per the skill's hard rule ("having a saved-but-not-pushed article is much better than having a researched-but-not-written run"), the article is on disk and recoverable.
**Recovery command** (operator can run after review):
```bash
cd /home/ubuntu/.hermes/workspace/website
# Re-run humanize check + em-dash density verification
...
# Insert sitemap entry above 06-06
# Insert news card at top of news/index.html
...
- **chinahospitalsguide remote is now on SSH** (was HTTPS) — the GitHub auth failure pattern that caused 4 consecutive missed pushes (06-03, 06-04, 06-05, 06-06) is resolved.
- **06-06 article is live** (3,481 words, em-dash density 19.4/1200, 0 banned vocab).
- **06-07 article is on disk**, awaiting publish — should be picked up by the next cron run or by the recovery command above.
```
