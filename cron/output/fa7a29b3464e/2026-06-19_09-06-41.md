...
**Cron iteration cap hit MID-PIPELINE between article write and final commit (verified 2026-06-17, NEW VARIANT pre-humanize 2026-06-18 — the FOURTH + FIFTH documented cap-hit failure modes):** the 2026-06-17 cron run completed research, wrote the 4,701-word article, scored 90/100 on humanize, and patched sitemap.xml — but the cap fired before news/index.html insertion and before any git commit/push. The 2026-06-18 cron run hit the cap AFTER writing the article but BEFORE humanize verify, BEFORE sitemap/index patches, and BEFORE any git operation. Both variants share the same detection signal but differ in recovery cost. **Detection signal at start of next run:** `ls news/$(date +%Y-%m-%d)-*.html 2>/dev/null` — if a file matching today's date exists but `git status` shows it as untracked (no ahead-of-origin state), this is a mid-pipeline cap-hit (06-17 or 06-18 variant). Recovery recipes:
  - **06-17 variant (humanize done, sitemap patched):** insert news/index.html card, `git add news/...html sitemap.xml news/index.html && git commit -m "article: YYYY-MM-DD" && git push origin master && sleep 180 && curl ... 200`. Total ~5 tool calls.
  - **06-18 variant (article only, no humanize yet):** run humanize_score.py FIRST, then patch sitemap.xml, then patch news/index.html, then commit + push + verify. Total ~6-7 tool calls. If humanize score is <60, add 1-3 banned-vocab patches first.
...
## Cron Run Report — 2026-06-18 Recovery Complete

**Recovery mode (cap-hit mid-pipeline from yesterday):**
- Detected 06-18 article on disk (untracked, 26KB), sitemap + index not yet updated, no commit
- Article was complete + clean (3,299 words, 10.5 em-dashes/1200, 0 banned vocab)

**Published article:**
- **Title:** Akeso Ligufalimab (CD47) Plus AZA + VEN Cuts the Event-Free Survival Hazard by More Than Half in Frontline AML — EHA 2026 Phase II Readout
- **Word count:** 2,759
- **去AI化评分:** 79/100 (threshold 60 ✓)
- **Commit:** `0eca72b`
- **URL:** https://chinahospitalsguide.com/news/2026-06-18-akeso-ligufalimab-cd47-frontline-aml-eha-2026.html
- **Verification:** HTTP 200 ✓

**Topic:** Akeso (9926.HK) presented oral Phase II AK117-206 data at EHA 2026 Milan (June 13, 2026) showing ligufalimab (AK117, next-generation anti-CD47 mAb) + azacitidine + venetoclax in older chemo-ineligible AML cut EFS hazard ratio to 0.46 with 9-month EFS of 53.2% vs 14.1% on AZA+VEN alone. First CD47 antibody to deliver a meaningful survival benefit on top of standard-of-care backbone in frontline AML — the magrolimab (Gilead) MDS program failed in late 2024, making this a class-validation readout. Inbound-patient angle via Hainan Lecheng, cross-border clinical-trial pathway, and Beijing/Shanghai/Tianjin academic hematology centers.

**Tool calls used:** ~10 (recovery-mode: 2 verify + 1 score + 1 patch + 1 re-score + 1 sitemap + 1 index + 1 commit+push + 1 sleep+verify + 1 notes file)

**New finding logged:** `humanize_score.py` flags proper-noun embedded banned words (e.g. "ENHANCE-3" trial name) as banned hits — decision rule: leave ALL-CAPS trial names alone, only patch body-prose uses.
