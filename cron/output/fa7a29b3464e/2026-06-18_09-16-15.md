...
```
RuntimeError: ## Cron Run Summary — 2026-06-18

### 06-17 Recovery (COMPLETED ✓)
Detected failure mode #4 (mid-pipeline cap-hit) on the 06-17 Akeso AK138D1 article. The article (4,701 words, humanize 90/100) was already on disk and the sitemap was patched, but `news/index.html` was missing the entry and no commit/push had happened.

- Inserted 06-17 card into `news/index.html` (before the 06-14 entry)
- Committed as `8188c41` and pushed to `origin/master`
- Verified HTTP 200 at https://chinahospitalsguide.com/news/2026-06-17-akeso-ak138d1-her3-adc-ivonescimab-breast-cancer.html

### 06-18 Fresh Article (PARTIAL — hit iteration cap before commit)
**Topic:** Akeso ligufalimab (AK117) — next-generation anti-CD47 monoclonal antibody — Phase II AK117-206 data in frontline chemo-ineligible AML, presented orally at EHA 2026 in Milan (June 13, 2026). Strong de-dup: 0 matches across the news library for `ligufalimab|AK117|CD47|AML|EHA`.

**Key data points (from PR Newswire via finanznachrichten.de, June 17, 2026):**
- EFS hazard ratio 0.46 (54% reduction in event risk)
- Median EFS: 9.1 vs 6.9 months
- 9-month EFS: 53.2% vs 14.1%
- 9-month OS: 78.7% vs 43.1%
- Composite CR rate: 56.7% vs 53.3%
- MRD-negative CRc: 46.7% vs 36.7%
- Median CRc duration: 10.4 vs 5.6 months

**Article written:** `news/2026-06-18-akeso-ligufalimab-cd47-frontline-aml-eha-2026.html` (26,654 bytes, 9 sections, complete with JSON-LD, data table, sources block, CTA box).

### What I Did NOT Get To Before the Cap
This is the **same failure mode #4 as 06-17** — article is on disk but:
- sitemap.xml has NOT been patched with the 06-18 entry
- news/index.html has NOT been patched with the 06-18 card
- No git commit, no git push, no HTTP 200 verify

**Recovery recipe for next cron run (estimated 5 tool calls):**
1. `python3 /home/ubuntu/.hermes/skills/creative/programmatic-seo/scripts/humanize_score.py news/2026-06-18-akeso-ligufalimab-cd47-frontline-aml-eha-2026.html --site chinahospitalsguide --sitemap sitemap.xml` (verify score ≥ 60)
2. `patch` sitemap.xml to insert the 06-18 entry at top of urlset
3. `patch` news/index.html to insert the 06-18 card before the 06-17 entry
4. `git add news/2026-06-18-*.html sitemap.xml news/index.html && git commit -m "article: 2026-06-18 Akeso ligufalimab CD47 + AZA + VEN Phase II AML EHA 2026" && git push origin master`
5. `sleep 180 && curl -s -o /dev/null -w "HTTP %{http_code}\n" https://chinahospitalsguide.com/news/2026-06-18-akeso-ligufalimab-cd47-frontline-aml-eha-2026.html`

### Detection signal for next run
```bash
ls news/2026-06-18-*.html 2>/dev/null  # non-empty → recovery mode
git status  # article file will show as untracked
```

### Report
- **热点标题:** Akeso Ligufalimab (CD47) Plus AZA + VEN Cuts EFS Hazard by More Than Half in Frontline AML — EHA 2026 Phase II Readout
- **字数:** ~3,400 words (file size 26,654 bytes)
- **去AI化评分:** Not yet scored (cap hit before humanize verify)
- **发布URL:** https://chinahospitalsguide.com/news/2026-06-18-akeso-ligufalimab-cd47-frontline-aml-eha-2026.html (NOT YET LIVE — pending recovery)
- **验证状态:** Pending — article on disk, push not executed
```
