...
| 项目 | 详情 |
|---|---|
| **文章标题** | Xia Zhi 2026: Summer Solstice Feng Shui for the Longest Day |
| **字数** | 2,391 词 (body) |
| **去AI化评分** | **95 / 100** (阈值 60) — 唯一遗留 note: high word count (符合站点日更文章 1800-2600 词范围) |
| **发布的 URL** | https://oriental-destiny.com/fate-2026-06-12.html |
| **部署状态** | HTTP 200 · 已上线验证 ✓ |
| **Git 提交** | `d2aa3d1` on `main` · pushed to `origin/main` |

### 主题选择
按内容日历，6 月 primary 主题是 **Summer Feng Shui / Fire Element**。今天选 **Xia Zhi (夏至) Summer Solstice 2026** —— 6/21 夏至是 6 月最关键的时间锚点，恰好衔接昨天 Bing Day Master 的"Fire element 顶峰"主题，并打开"夏至是 Yang 顶峰但 Yin 已开始"的转折叙事，自然过渡到 7 月 Xiao Shu。

### 文章结构 (5 段 H2 + 2 段 H3)
1. **What Xia Zhi is, in the classical calendar** — 定义 + 与 Xiao Man / Xiao Shu 的位置关系
2. **Why the longest day is also the start of Yin** — Yin-Yang 理论里"夏至是 Yang 顶峰也是 Yin 起点"的核心论点
3. **What to do in the home before the solstice** — 5 条可操作建议 (开南窗、补水元素、清理东南、棉麻床品、北侧阅读灯)
4. **What to avoid in the two weeks around the solstice** — 古典禁忌 (不装修、不在南面加重家具、不在南向房吵架) + 小户型适配
5. **Personal BaZi and the Summer Solstice** — 按图表元素分四类给出当周建议
6. **FAQ** — 6 题 (时机、术语区分、是否适合搬家、饮食、南半球适配等)

### Voice / Humanize 校验
- 第一人称 5 次，符合站点 Day Master 系列语气
- 句长标准差 9.3，自然变化
- Em-dash 密度 11.5 / 1200 词 (站点 baseline 10-18) ✓
- 0 个 `actually`、0 个 banned vocab、0 个 copula avoidance (`stands as / serves as`)
- 含 3 处 CJK 注释 (`节气` `夏至` `数九`) — 与站点已有文章 (bing-day-master.html 含 42 处非 ASCII) 风格一致
- 内部链接到 `li-chun-bazi-beginning-of-spring.html`、`fire-element-in-bazi.html`、`five-elements-explained.html`、`luck-cycles-da-yun.html`、`instant_reading.html` —— SEO 内部链接结构完整

### 流程备注
- 使用 `programmatic-seo` skill + `humanizer` skill
- 研究阶段发现 cron 提示中 `memories/layer3/research/competitor-research.md` 路径不存在；已用正确路径 `.hermes/memories/layer3/research/article_topics.md` 和 `terminology_mapping.md` 替代
- 分支: cron 提示说 `master` 但实际 remote HEAD 在 `main`，按 skill 警告推送到 `main`
- 远端: SSH URL `git@github.com:qzw-alt/oriental-destiny.git` 完好 (前序 cron 修复的成果)
- 全程 10 个工具调用，预算控制良好
