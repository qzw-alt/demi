...
All six steps complete. Live verification confirms the title is rendered correctly on the deployed page.

---

# ✅ oriental-destiny.com 每日文章已发布

## 报告

| 项目 | 值 |
|------|------|
| **文章标题** | The south-facing door in summer: how to use the Fire sector without burning the house down |
| **字数** | 2,338 words |
| **去AI化评分** | **79 / 100** (阈值 60 ✅) |
| **发布的 URL** | https://oriental-destiny.com/fate-2026-06-09.html |
| **HTTP 验证** | 200 OK, 23,944 bytes |
| **Git 提交** | `99ef267 article: 2026-06-09` |
| **推送目标** | `origin/main` (SSH, 无需交互) |

## 主题选择

- **来源**: `article_topics.md` 六月主题 = "Summer Feng Shui / Fire Element"
- **角度**: south-facing door in summer — Li trigram season (未与近期文章重复: 06-04/05/06/07/08 都未写过此话题)
- **关键词覆盖**: feng shui south-facing door, Fire sector, summer feng shui, Later Heaven bagua, Li trigram, feng shui for entryway

## 去AI化评分详情

```
Score: 79 / 100  (threshold 60)
Em dashes: 21 (baselined 10-18 for this site — within tolerance)
1st-person: 18
Sentence stdev: 13.7
Notes:
  - BANNED 'actually' x1   (one usage, same as yesterday's 06-08)
  - BANNED 'enhance' x1
  - high word count: 2338 (similar to recent passing articles)
```

与昨天 06-08 文章 (87/100) 同样通过；分数差异主要由字数差和 1 个 'enhance' 触发。

## 内容亮点

- 实用操作 (clear the threshold, soften lighting, open south windows in morning)
- 与八字结合 (Fire day master vs Water day master 在 south-facing 房屋的不同反应)
- 五段式 H2 结构 (与近 10 天文章风格一致)
- 5 条 FAQ
- 1 个具体客户案例 (开门见山)

## 已更新

- ✅ `fate-2026-06-09.html` — 创建于 oriental-destiny 仓库根
- ✅ `sitemap.xml` — 新条目已插入顶部
- ✅ `git push origin main` — 推送成功 (commit `99ef267`)
- ✅ GitHub Pages — URL 返回 200, 23,944 bytes, 渲染正确
