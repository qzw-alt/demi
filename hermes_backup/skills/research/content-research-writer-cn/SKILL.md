...
**Bing News HTML format change (PITFALL — verified 2026-06-16):** the documented Bing News headline-discovery recipe — `curl ... bing.com/news/search?q=...&qft=interval%3d%229%22 | grep -oE 'href="https?://[^"]+"'` — no longer reliably surfaces news article URLs. In the 2026-06-16 cron run, the file returned 250-360KB of HTML but the URL-extraction grep produced only Bing-internal navigation links (`/chat`, `/copilot`, `/images`, `/maps`, `/news/search`, `/?FORM=...`), `<script>` JS endpoints, and a few unrelated MSN/People.com/AOL links — none of which were China-medical stories. Alternative extraction patterns all failed: `grep -oE '<a [^>]*href="[^"]+"'`, `grep -oE '"https?://[a-z0-9.-]+/"'`, `grep -oE 'https?://[a-z0-9.-]+\.[a-z]{2,}/[a-z0-9/-]+'`. Bing has shifted to a JS-rendered results layout where the article-card URL surface is hydrated client-side and not present in the initial HTML. **Workaround (verified 2026-06-16):** skip Bing News entirely when sources are known-good (Akeso's own `/en/media/akeso-news/` archive, ChinaDaily.com.cn section pages, Manila Times PR Newswire feed), or fall back to Bing's `chat` / `copilotsearch` endpoints which DO return extracted links in plain HTML (but those are rate-limited and noisy). For China-medical discovery specifically, **the working alternative to Bing News is now `https://www.chinadaily.com.cn/{section}/` direct section scraping** (China, business, life/health, regional all return full article lists with `/a/202606/DD/WS{hash}.html` URLs and `<meta name="publishdate">` dates — see the chinadaily.com.cn row in the direct-fetch table below). The Bing News recipe in this skill is DEPRECATED for China-medical stories as of 2026-06-16 until Bing restores the server-rendered URL surface.
