# Cron Job: daily-medical-news

**Job ID:** 8e8ffb5faa96
**Run Time:** 2026-05-31 15:12:51
**Schedule:** 0 7 * * *

## Prompt

[IMPORTANT: The following skill(s) were listed for this job but could not be found and were skipped: content-research-writer-cn, programmatic-seo. Start your response with a brief notice so the user is aware, e.g.: '⚠️ Skill(s) not found and skipped: content-research-writer-cn, programmatic-seo']
...
⚠️ Skill(s) not found and skipped: `content-research-writer-cn`, `programmatic-seo`

---

## Cron Job Report — 2026-05-31

**Status:** Article written and committed locally; push to GitHub failed (credentials issue — see below).
...
| Field | Value |
|---|---|
| **Title** | Traditional Chinese Medicine Goes Global: How China Became a Wellness Destination for International Visitors |
| **Filename** | `news/2026-05-31-tcm-global-wellness-china-medical-tourism.html` |
| **Topic** | Rising international interest in TCM as medical tourism draw, sourced from People's Daily "Becoming Chinese" trend |
| **Approx. words** | ~1,888 |
| **Word count (body text)** | ~1,100 |
| **De-AI score** | ~72/100 |
| **AI patterns found** | 0 em dashes, 0 high-frequency AI vocabulary, 0 emojis, natural rhythm |
| **Sources cited** | People's Daily Online (May 27, 2026), China Daily |

---

### Changes Made
1. **Article created:** `news/2026-05-31-tcm-global-wellness-china-medical-tourism.html`
2. **news/index.html updated:** added article entry at top of news list
3. **sitemap.xml updated:** added `<loc>` entry for new article
4. **git commit created:** `b024cc0 "news: 2026-05-31"`
...
The repository remote uses **HTTPS** (`https://github.com/qzw-alt/chinahospitalsguide.git`) but no GitHub token or SSH key is configured in this cron environment. The `gh` CLI is also not installed. The commit was created successfully but **could not be pushed**.

**Fix required:** Add one of the following to the cron job environment:
- A GitHub personal access token in `~/.netrc` (`machine github.com login <TOKEN>`)
- An SSH key registered with GitHub (then switch remote to `git@github.com:qzw-alt/chinahospitalsguide.git`)
...
Expected: `https://chinahospitalsguide.com/news/2026-05-31-tcm-global-wellness-china-medical-tourism.html`

> **Note:** The article and all index/sitemap updates are locally committed but not yet on GitHub Pages. Once credentials are fixed, run `git push origin master` in `/tmp/demi_backup/workspace/chinahospitalsguide/` to deploy.
