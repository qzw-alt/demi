...
description: "Gather real-time web data (news, trends, stats) via terminal + curl to free APIs, when no web_search/web toolset is available. Covers HN Algolia, NYT RSS, Techmeme feed.xml aggregator, DuckDuckGo/Bing HTML scraping fallback chain, and source-verification discipline for daily briefings."
version: 1.6.0
metadata:
  hermes:
...
**Tip for daily briefings**: Run a parallel batch of narrow single-keyword queries (`Anthropic`, `OpenAI`, `DeepSeek`, `Mistral`, `Google Gemini`, `Llama`, `GPT-5`, `Claude`, `AI funding`, `AI raise`, `AI regulation`, `Launch HN`) with the same date filter — HN Algolia is fast enough that 8–12 single-keyword queries in one batched `terminal()` call take ~3s and give you much better category coverage than one broad `query=AI`. **NEVER use OR operators in the query parameter — see the validated pitfall below.** Combine with `search` (relevance/points-weighted) + `search_by_date` (chronological) for both "what's hot" and "what just dropped". **Default query list for AI briefings (validated 2026-07-26):** `Anthropic`, `OpenAI`, `Google+Gemini`, `DeepSeek`, `Mistral`, `Llama`, `GPT-5`, `Claude`, `AI+funding`, `AI+raise`, `AI+regulation`, `China+AI`, `Series+raise`, `valuation+billion`. Dedupe all hits by `objectID` and sort by `points` desc for category leaders.

**Pitfall — `query=A+OR+B+OR+C` returns 0–1 hits (validated 2026-07-16, re-confirmed 2026-07-26)**: Multi-OR queries in the Algolia `query` parameter are unreliable. `query=GPT+OR+Claude+OR+Gemini+OR+Llama+OR+DeepSeek&tags=story&numericFilters=created_at_i%3E{TS}` returned exactly 1 hit while the same query without `numericFilters` returned 25+. The combination of the OR operator with a date filter seems to defeat relevance scoring. **Fix (now standard practice, not optional)**: split into separate single-keyword queries per topic/company (`query=Anthropic`, `query=OpenAI`, `query=DeepSeek`, `query=Meta+AI`, `query=Mistral`, `query=China+AI`, `query=AI+funding`, `query=AI+raise`, `query=Google+Gemini`, `query=Llama`, `query=GPT-5`, `query=Claude`) and dedupe hits by `objectID` in Python. **This is slower but reliable, and per-keyword queries give much better category coverage for briefings anyway** — the OR shortcut isn't worth the risk of returning 0 hits on a date-filtered query. 8–10 single-keyword queries in one batched `terminal()` call take ~3s total. Single-keyword queries also return the `points` field reliably when using the `search` endpoint (vs. `search_by_date` which strips points) — combine both: use `search_by_date` for chronological triage, then re-query top hits via `search` for popularity scores.
...
| **TechCrunch article pages** | `https://techcrunch.com/{YYYY}/{MM}/{DD}/{slug}/` | SSR HTML | **Returns full body via curl as of 2026** — article text in `<p>` tags inside `<div class="entry-content">`. Earlier "JS-rendered" advice was outdated. **Slug-discovery pitfall (validated 2026-07-28):** the per-day listing page (`techcrunch.com/{YYYY}/{MM}/{DD}/`) returns titles with truncated slugs in the `<a href>` — guessing the full slug from the listing often 404s ("We're sorry, we seem to have lost this page"). To recover the canonical slug: `curl -sL "https://techcrunch.com/?s=KEYWORD" -A "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36"` and extract the first matching link. The search-page `<a href>` is the canonical slug. Same trick works on Anthropic (`/news/` listing returns all `/news/{slug}` links), HuggingFace (`/blog` returns `/blog/{slug}` links), and the HF blog directly. |
| **Techmeme feed.xml** | `https://www.techmeme.com/feed.xml` | RSS/Atom | **Strongest single-source aggregator for daily tech/AI briefings (validated 2026-07-28).** Returns ~15 curated items/day with `<title>`, `<description>` (already HTML-cleaned by the aggregator and prefixed with the source outlet name, e.g. "Zac Hall / 9to5Mac :"), and `<pubDate>` in `-0400` EDT. Mixes TechCrunch, NYT, Reuters, Bloomberg, Wired — saves 3–4 individual curl calls. Parse with `re.findall(r'<item>(.*?)</item>', xml, re.DOTALL)` then strip residual HTML. **For AI briefings, this is the highest signal-to-noise single endpoint** — better than HN Algolia for breadth, better than NYT RSS for tech/AI relevance, better than Bing for source diversity. Use as the **primary step after primary-source company blogs**. Note: occasional items are finance/macro not AI-relevant — filter by keyword in title/description. |
...
- **Techmeme feed.xml is the single best aggregator for daily AI briefings (validated 2026-07-28)**: `https://www.techmeme.com/feed.xml` returns ~15 curated items/day with editorial-summary-grade descriptions (already HTML-cleaned, prefixed with `Source / Outlet :`), pubDate in EDT, and source diversity baked in (NYT, Reuters, Bloomberg, Wired, TC). For a cron-style briefing run, **call Techmeme feed.xml FIRST** (after primary-source company blogs) — it replaces 3–4 individual curl calls to NYT RSS, TechCrunch archive, and HN front page. Parse with `re.findall(r'<item>(.*?)</item>', xml, re.DOTALL)` then strip residual HTML from descriptions. Combine with HN Algolia `search_by_date` (with date filter) to fill breadth gaps; Techmeme is editorial-curated, not exhaustive.

- **TechCrunch slug discovery via internal search (validated 2026-07-28)**: Per-day archive pages (`/2026/07/27/`) return truncated slugs that 404. To recover canonical slugs: `curl -sL "https://techcrunch.com/?s=KEYWORD" -A "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36"` and extract the first matching `<a href>`. Same pattern works on Anthropic (`/news/` listing returns full slugs), HuggingFace (`/blog` returns full slugs). The canonical-slug sources are the safe entry points; the per-day archive is a fallback only.

- **Microsoft blog endpoints are Cloudflare-gated from curl (validated 2026-07-28)**: `microsoft.com/en-us/security/blog/{slug}` and `blogs.microsoft.com/{slug}` both return "Attention Required | Cloudflare" shells. Don't waste curl turns on Microsoft blog URLs in briefings — use TC + Techmeme coverage of MS announcements, and the official Microsoft AI social handles for sourcing.

- **Cron job stdout is not visible to user, so the briefing must go in the assistant final response (validated 2026-07-28)**: When running as a cron-delivered job, do NOT try to deliver via `send_message` (tool unavailable). The final assistant response IS the deliverable — keep it clean and self-contained markdown. The skill should call this out explicitly so the next cron run doesn't waste turns looking for a delivery mechanism.
...
- `references/ai-briefing-format.md` — the exact "AI行业每日简报" format spec (6 sections, one-sentence-per-item, emoji, ≤500 chars, HN-verified numbers only), with worked templates from 2026-07-16 and 2026-07-26. Includes the verification tier rubric (Tier 1–4) for picking items, hedge-language patterns for Tier 3 items, per-section length budgeting, and a list of which major-publication URLs return Cloudflare JS gates (use HN title only). Load when the user asks for a daily AI briefing.