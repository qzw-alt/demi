...
**SSH as the most reliable fallback:** If the machine already has an SSH key registered with GitHub (check `ls ~/.ssh/id_ed25519.pub`), the SSH URL approach is simpler and more reliable than the PAT+helper chain:

```bash
# Use SSH URL — bypasses PAT auth entirely when keys are deployed
git remote set-url origin git@github.com:owner/repo.git
git push origin main
```

This succeeds even when:
- Fine-Grained PATs are rejected from HTTPS URL embedding
- netrc has stale tokens
- credential helper is bypassed in restricted sandbox environments

**When to prefer SSH over PAT+helper:**
| Scenario | Recommended approach |
|----------|---------------------|
| Machine has SSH key registered on GitHub | SSH URL — most reliable |
| Machine has no SSH key, PAT available | PAT + `gh auth git-credential` helper |
| gh CLI installed | `gh auth git-credential` with `GH_TOKEN` env var |