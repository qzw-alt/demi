...
- `references/cron-failure-manual-recovery.md` — **load this when `last_status=error` for today's cron and the `[SHIPPED_OK]` token is missing from the output file.** Verified 2026-07-14 (chinahospitalsguide.com `daily-chg-medical-news` cap-hit, manually recovered in ~12 tool calls). Covers the 4 cap-hit variants (research-completed-no-article, mid-pipeline-uncommitted, post-commit-no-push, false-positive) with detection signals, per-variant recovery recipes with explicit tool-call budgets, and the long-term structural fix (max_iterations bump vs prompt hard-ordering).
