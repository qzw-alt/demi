...
**Cron iteration cap hit MID-PIPELINE between article write and final commit (verified 2026-06-17 + 2026-06-18 + 2026-06-19 + 2026-06-20 + 2026-06-28 — the FOURTH, FIFTH, SIXTH, SEVENTH, EIGHTH documented cap-hit failure modes):** the 2026-06-17 cron run completed research, wrote the 4,701-word article, scored 90/100 on humanize, and patched sitemap.xml — but the cap fired before news/index.html insertion and before any git commit/push. The 2026-06-18 cron run hit the cap AFTER writing the article but BEFORE humanize verify, BEFORE sitemap/index patches, and BEFORE any git operation. The 2026-06-19 run recovered the 06-18 state successfully. The 2026-06-20 run hit the cap AFTER writing the article, AFTER patching both sitemap.xml and news/index.html, but BEFORE the git commit — the cleanest partial state yet (all 3 file changes ready, just uncommitted). The 2026-06-28 run hit a NEW variant: cap fired DURING the humanize loop after 3 `actually`-in-H2 patches and 8 em-dash-insertion patches, with density still at ~9/1200 (well under the 17-23 baseline), sitemap/index unpatched, no commit, no push. See the 06-28 variant section below for the recovery recipe. **Detection signal at start of next run:** `ls news/$(date +%Y-%m-%d)-*.html 2>/dev/null` — if a file matching today's date exists but `git status` shows it as untracked (no ahead-of-origin state), this is a mid-pipeline cap-hit (06-17 / 06-06-18 / 06-19 / 06-20 / 06-28 variant). **Verified quint-variant state matrix (as of 2026-06-28):**
...
| 06-20 | YES | YES | NO | ~5 calls (commit + push + rebase + verify) |
| 06-28 | NO | NO | NO | ~7 calls (verify + finish humanize + sitemap + index + commit + push + verify) |
...
**06-26 yin/yang run in ~10 tool calls (verified 2026-06-26, oriental-destiny.com Yin and Yang, score 80/100, no sibling-cron divergence, score/audit divergence 35 pts):** the 06-26 run followed the 06-15/06-16 recipe with two new wrinkles. Tool breakdown: (1) `terminal` — `ls fate-2026-06-26-*.html` + `git status` + `git remote -v` (combined) — no pending article, clean tree, SSH remote. (2) `read_file` — `article_topics.md` + `terminology_mapping.md` (combined) — research. (3) `terminal` — `grep -lE "(yin and yang|yin yang)"` against all `fate-*.html` + `grep -E "<title>" fate-2026-06-2[0-5].html | head -10` (combined) — confirmed no dedicated pillar piece + surveyed recent thread. (4) `read_file` — `fate-2026-06-25.html` (offset 160-440) — voice + scaffolding reference (specifically the Xun cell article, since the new piece threads off the prior wealth-corner article). (5) `write_file` — `fate-2026-06-26.html` — 3,019-word article. (6) `terminal` — `grep -P '[^\x00-\x7F]' …` (non-ASCII check) + `python3 scripts/humanize_score.py …` + `python3 scripts/humanize_audit.py …` (combined) — score 80/100, audit 45/100, 35-pt divergence caused by 9 `features` false-positives. (7) `patch` — removed `actually` from H1 heading (per 06-22 rule, headings never tolerate `actually`). (8) `patch` — removed `actually` from hero lead (per 06-22 rule, lead reads as AI-tell if it leans on the word). (9) `terminal` — re-scored (80/100 confirmed, audit cleaner). (10) `patch` + `terminal` — sitemap update + `git config user.email/name` + `git add … && git commit -m "article: 2026-06-26"` + `git push origin main` (combined into one terminal call after the sitemap patch). (11) `terminal` — `sleep 180 && curl --max-time 30 -s -o /dev/null -w "HTTP %{http_code}\n" …` — HTTP 200 verified. **Wrinkle 1 (audit/score divergence):** the 06-26 run was the first time the two scripts disagreed by 20+ points. The audit's COPULA_AVOID list includes `r"\bfeatures\b"`, which fires 5-15 times per technical article in the noun sense ("the room's measurable features"). The site-aware score is correct (80); the audit is the noisy one (45). See `references/humanize-audit-false-positives.md` for the full false-positive catalog. **Wrinkle 2 (`actually` in H1):** the 06-22 rule ("actually in headings is NEVER a false positive") scaled to H1 too — patching the H1 `What the Two Sides Actually Mean` → `What the Two Sides Mean` brought the audit to 0 AI-vocab hits without touching any of the 4 remaining body `actually` uses. Rule update: the "actually in headings" prohibition covers H1, H2, AND H3.
...
**Referenced-but-never-covered pivot (NEW pattern, verified 2026-06-19, second confirmation 2026-06-26):** after the first pivot lands (e.g. the Annual Flying Stars article on 06-18), the next day's pivot can mine the just-published article for terms that were REFERENCED but never given their own pillar piece. Verified example from 2026-06-19: the 06-18 Flying Stars article's FAQ mentioned "a luo pan gives the precise reading" and the 06-18 body mentioned "the Compass school." Neither had a dedicated article, but both were referenced as if the reader already knew them. The 06-19 article ("Luo Pan: How to Read the Feng Shui Compass") became the first-ever dedicated piece on the luo pan, with a seasonal bridge to Xia Zhi (Summer Solstice on 06-21) which is one of the four classical annual checkpoints for taking a compass reading. **Second confirmation 2026-06-26:** the 06-24 Bagua Map and 06-25 Wealth Corner articles both used the word "yin/yang" repeatedly without a dedicated pillar piece. The 06-26 article ("Yin and Yang in Feng Shui") became the first-ever dedicated piece on yin/yang, with a seasonal bridge to the Li Qiu pivot (August 7, 2026) where the Fire Month's yang-heavy weather gives way to the Earth Month's more balanced polarity. **Recipe:** after publishing an article, scan it for technical terms that appear once or twice as parenthetical references (e.g. "the Compass school", "the luo pan", "He Tu / Fu Xi diagrams", "the 28 lunar mansions", "yin and yang", "five elements", "the I Ching"). `grep -lE "(TERM)" *.html` — if 0 matches against standalone articles but ≥1 match inside another article's body, that term is a "referenced-but-never-covered" pivot target. The article then naturally links back to the article that referenced it, and the seasonal bridge gives the timing ("two days before Xia Zhi", "the autumn checkpoint before Li Qiu", "the late-June pivot before Earth Month"). Best for the article immediately after a pivot, when the reader is still in pivot-mode and expects a foundational-concept deep-dive. **Thread-continuity bridge sentence pattern (verified 2026-06-26):** the lead paragraph of the pivot article should explicitly reference the prior articles in the thread, e.g. "Earlier this month the Fire Month articles leaned on the word 'yang' without ever defining it, and the wealth corner piece from two days ago leaned on the word 'yin' the same way." This single sentence does three things: (1) re-anchors the prior thread for readers who landed directly on this article, (2) signals to the reader that this is part of a coherent series, not a random piece, and (3) gives the article a natural reason to exist (the prior articles assumed the term was already known, this one fills the gap). Pattern generalizes to any "referenced-but-never-covered" pivot.
...
3. **Inspect:** are any of the new remote commits touching the same files you're about to commit (news/, sitemap.xml, news/index.html)? If yes, expect a rebase conflict. If no (the case here), the rebase will be clean.
...
**Why it works:** the cron runs are append-only on the article side (one new article per day, one new sitemap entry, one new index.html card). If the new remote commits are touching project meta files (MEMORY.md, AGENTS.md, .github/, scripts/, page meta tags) rather than the article/sitemap/index files, the rebase is a clean cherry-pick. **Detection signal:** the failed push itself is the natural signal. The first `git push origin master` returns non-zero with "fetch first" hint. The 1-2 extra call cost is acceptable as long as the cron budget has the headroom (5+ remaining tool calls when entering Step 6). **Why this wasn't a problem before 2026-06-21:** earlier cron runs either pushed on the same day as the commit (no gap for the remote to advance) or hit the cap mid-pipeline (no commit to push). The 06-20 → 06-21 day-gap is the first recovery where the remote had time to receive new commits from another source (a human operator doing project-meta work, or a parallel cron). **Mitigation (optional, future runs):** add `git fetch origin master` to Step 0 pre-flight and `git log --oneline HEAD..origin/master` to detect this state early — if non-empty, the recovery script knows to expect a rebase before the push.
