...
## Quick reference: pitfall pointers

For the full text of recurring pitfalls, see the support-file index in `references/`. Key pointers (verified across recent cron runs):

- **`@@type` JSON-LD typo and the sibling `"@type": "Text"` FAQPage typo + `replace_all` destruction:** `references/json-ld-typo-and-replace-all-pitfalls-2026-07.md`
- **Eleventy `.md` source files render to trailing-slash URLs (NOT `.html`); sitemap `<loc>` convention:** `references/eleveny-md-render-trailing-slash-2026-07.md`
- **Cap-safe cron ordering (commit+push BEFORE humanize loop):** inline below in "Hard rule #2"
...
**Element-parallel outdoor-indoor handoff (NEW pattern, verified 2026-07-30, oriental-destiny Outdoor Lighting → Indoor Lighting):** see `references/element-parallel-handoff-2026-07.md` for the full pattern, detection recipe, mirrored-section recipe, and what-can-go-wrong list. Summary: a specialization of the teed-up-next-article signature mechanism — when the last outdoor-feature article in a property scan ends its body with a list of **2-3 named indoor-side follow-up topics** for the SAME element, the next day's cron run picks up the FIRST named hook and writes the indoor-side twin with the same typology framework, mirrored structural sections, threshold-continuity sentence at the door, and chart-pair + seasonal-shift cross-check preserved. Verified on 2026-07-29 → 2026-07-30 (Outdoor Lighting 5,129 words, 89/100 → Indoor Lighting 4,470 words, 89+95/100). Distinct from the room-walk continuation (which moves the boundary into a new room with a different element); the element-parallel handoff reads the **same element** on both sides of the wall as a two-part sequence. Future candidates in the site's content calendar: plants, water, sound, smell (see the reference for which pairs are viable).
...
3. `git pull --ff-only origin master` — fast-forward to origin HEAD (sitemap now matches origin's new convention exactly)
...
The full playbook (decision rules, phase-by-phase recipes, pitfalls, tool-call budget, verification recipes) lives at `references/content-matrix-overhaul.md`. Companion files:
...
**JSON-LD FAQPage `"@type": "Text"` typo + `replace_all=true` destruction pitfall (NEW — verified 2026-07-29):** the FAQPage sibling of `@@type` above. The correct `@type` for `acceptedAnswer` is `"Answer"`, not `"Text"`. When the typo appears N times in N different surrounding contexts (5 FAQ entries is typical), the natural fix is `replace_all=true`. **Don't.** The patch tool's regex matches the key-value pair PLUS the trailing text content as one continuous token, then replaces it with just the corrected key — silently destroying the answer text. The 07-29 cron run hit this on a 5-entry FAQPage block; recovery took 6 tool calls (1 detection + 1 destructive patch + 5 individual restore patches). **Decision rules** (full transcript in `references/json-ld-typo-and-replace-all-pitfalls-2026-07.md`): (a) for typo-in-N-different-contexts, do N individual `patch` calls with enough surrounding context to make each unique; (b) for typo-in-N-same-context, `replace_all=true` is safe; (c) `replace_all=true` is NEVER safe for JSON-LD key-value pairs where the value has trailing content on the same line. **Detection:** after any `write_file` or `patch` that writes a JSON-LD block, run `grep -E '"@type": "(Text|FAQPage|Question|Answer)"' FILE` and confirm exactly 1 `FAQPage`, N `Question`, N `Answer`, 0 `Text` entries.

**Eleventy `.md` source files render to trailing-slash URLs, NOT `.html` (NEW — verified 2026-07-29, chinahospitalsguide.com):** when Eleventy renders a `.md` source file like `blog/2026-07-29-slug.md`, the live URL is `https://chinahospitalsguide.com/blog/2026-07-29-slug/` (directory-with-trailing-slash), NOT `.html`. The old `news/*.html` flat-file convention is separate from this. **Sitemap pitfall:** when patching `sitemap.xml` to add a new `.md`-source article, the `<loc>` entry must use the trailing-slash directory URL, NOT `.html`. The 07-29 cron run wrote `.html` in the sitemap and got HTTP 404 on the canonical `<loc>` URL (the article was still live via canonical `<link>` and JSON-LD `mainEntityOfPage`, but the sitemap is a soft 404 to crawlers). **Decision rule:** for `.md`-source articles, the sitemap entry is `<loc>https://SITE/blog/YYYY-MM-DD-slug/</loc>` with trailing slash. **Detection:** `curl --max-time 25 -I https://SITE/blog/YYYY-MM-DD-slug/` should return HTTP 200; the `.html` URL will return HTTP 404. **Full state matrix and recovery recipe:** `references/eleveny-md-render-trailing-slash-2026-07.md`.
