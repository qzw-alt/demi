...
Sections 2 (data-box callout) and the closing section (medical-tourism translation with bulleted patient questions) are the consistent differentiators across all six archetypes — without them, every article reads as a press-release paraphrase and the humanize score drops 15-20 points.
...
| **07-04 (chinahospitalsguide, this run's case)** | YES | YES | NO | ~10 calls (Step 0 detect + verify state + humanize + commit + push-rejected + fetch+log+rebase + push-succeed + sleep+curl-404 + origin+last-modified diagnose + pending-note write) |
