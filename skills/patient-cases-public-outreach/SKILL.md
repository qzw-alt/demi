...
**触发器识别**：伟烨说"**直接发我/我直接转发/复制用/改好发我/不用解释**" → 触发**纯文本发布模式**：

- 文案正文**禁止附任何注**（⚠️/要不要改/数字确认）——所有沟通放到飞书对话单独发。
- 文案**末尾不留 "要不要我再出一版"** 之类的勾选话术——他转不出去。

#### ⚠️ "复制用" 模式的强包装规则（2026-07-25 拍板升级）

**核心**：用户已经不止一次**反复强调**"只发内容，其他不用说了"——不是礼貌，是**真正的偏好**。任何起稿任务（邮件 / 微信 / 朋友圈 / 报告附言）默认进入这个模式。
...
- ❌ 上方"以下是给你的邮件/微信内容" 中文说明
- ❌ 下方"⚠️ 复制规则"、"客户姓名占位符 [name]"、"复制到微信"等提示
- ❌ "✓ 我做的关键事 / ❌ 我没做的事 / 📋 这封邮件的策略说明"等战略总结
- ❌ emoji 装饰（🚨 ⚠️ ✅ ❌ 全部不要）—— **除非用户明示加**
- ❌ "你下一步："、"你拍一下" 之类的钩子 —— 那是给用户的对话，不是给客户的文案
- ❌ "如何复制"、"记得替换占位符" 等使用说明 —— 用户知道怎么复制

**可以加的**（且应该加的）：

- ✅ **占位符替换提醒**（如果文案里有 `[name]` 或 `[医院名]`，**单独**说"⚠️ 记得替换占位符"——但不要放在复制框里）
- ✅ **如果文案里含 schema bug 修复 / 敏感信息过滤** → **单独**在复制框外说明

**判断标准**：**复制框里必须是 100% 可以无脑复制粘贴到客户场景**的成品。任何"提示用户"的内容都不进框。

**对比示例**：

```
❌ 错（太多解释）：
好的，这是给你的邮件内容（写得有点长，但诚意到位）：
[邮件正文...]
⚠️ 注意：客户姓名占位符 [name] 你发的时候替换一下。
📋 我做的 4 件关键事：
1. 不评论放化疗
2. ...
...
**这种格式所有"邮件/微信/朋友圈文案/报告附言"起稿任务都要遵守**，不分渠道，不分长度。
