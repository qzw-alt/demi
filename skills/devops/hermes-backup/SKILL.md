...
**Protect repository housekeeping at the rsync boundary.** The cleanest defense is to exclude `.gitignore` and `.gitattributes` from the source mirror, while still writing the canonical `.gitignore` unconditionally after cleanup. If either file is tracked in `HEAD`, save its blob before rsync and restore it immediately afterward. This avoids relying on timing-sensitive checkout guards:

```bash
mkdir -p /tmp/hermes-backup-housekeeping
for f in .gitignore .gitattributes; do
  git cat-file -e "HEAD:$f" 2>/dev/null && git show "HEAD:$f" > "/tmp/hermes-backup-housekeeping/$f"
done
```

Add these patterns to the rsync command:
...
```bash
for f in .gitignore .gitattributes; do
  [ -f "/tmp/hermes-backup-housekeeping/$f" ] && cp -p "/tmp/hermes-backup-housekeeping/$f" "$f"
done
```
...
# Provider-prefix-aware inline redaction (NEW — added 2026-07-19; fixed 2026-07-24):
# When the byte scan flags a token whose prefix the walker doesn't cover
# (e.g. tvly-, glpat-, AIza, xoxb-, AIzaSy), use the BUNDLED script
# `scripts/redact_providers.py` — it handles tvly, sk, github_pat, ghp, gho,
# ghu, ghr, ghs, AIza, glpat, xoxb, xoxp in one pass.
#
# ⚠️ BUG FIXED 2026-07-24: the original 2026-07-19 v1 snippet had TWO bugs:
#   (1) body character class was `[a-zA-Z0-9_]+` (missing `-`), so on
#       `tvly-dev-sAFTx-...` it matched only `tvly-dev` (8 chars),
#       short-circuited the body-length gate, and silently reported NO CHANGE
#       even when a 62-char full key was in the file.
#   (2) the `\b` boundary at the start of the pattern broke at the first
#       internal hyphen — wrong direction for provider tokens whose body
#       contains hyphens throughout.
# Fixes:
...
#   - Operates on a SINGLE file path with NO prefix argument — runs all
#     known prefixes in one pass.
#
# Usage:
#   python3 <skill_dir>/scripts/redact_providers.py "workspace/website/MEMORY.md"
#   # → truncated N tvly- token(s) in workspace/website/MEMORY.md
#   # → WRITTEN: workspace/website/MEMORY.md
# Re-run the relevant scan block (grep) — loop until clean.
#
# ⚠️ CRITICAL: the bundled `scripts/redact_secrets.py` single-file mode has
# the SAME coverage gap as the batch walker — calling it on a known tvly-
# leak file redacts `sk-` but leaves `tvly-` untouched (verified 2026-07-24:
# single-file mode printed "REDACTED: 1 sk-" but the working tree still
# contained `tvly-dev-sAFTx-...a7Si` in full). Therefore when the walker flags
# a tvly- / glpat- / AIza- leak, ALWAYS chain-call redact_providers.py
# after the bundled single-file script — the two scripts are complementary,
# not substitutable. Future-proofing: if a walker gap on a NEW prefix is
# discovered, patch BOTH the bundled `redact_secrets.py` AND this skill's
# `redact_providers.py` in the same change — otherwise the next backup
# will repeat the gap.
```
...
**Run provider-aware redaction and full scans as a gate, not merely as diagnostics.** The primary walker does not cover every provider prefix. After `redact_secrets.py`, run `scripts/redact_providers.py` against every path flagged by the length/all-file byte scan, then loop until clean. After staging, run the same byte patterns against index blobs and refuse the commit on any hit. Re-run the staged scan immediately before `git commit` so no subsequent edit bypasses the gate.

Also reject accidental staging of excluded content and housekeeping damage:

```bash
# Pseudocode/guard requirements before commit:
# - zero staged paths under canonical excluded prefixes/exact paths
# - .gitignore and .gitattributes exist
# - neither housekeeping file has status D or ??
# - staged-blob secret scan returns zero hits
```
...
```bash
# Confirm the remote really has the exact local commit over the configured SSH origin.
test "$(git remote get-url origin)" = 'git@github.com:qzw-alt/demi.git'
git fetch origin master
LOCAL=$(git rev-parse HEAD)
TRACKING=$(git rev-parse origin/master)
...
Do not report success from `git push` output alone. Require local `HEAD`, fetched `origin/master`, and `git ls-remote` to agree; then verify the worktree is clean.
...
- `references/2026-07-22-session-notes.md` — clean run: 7th CJK miss confirmed, 4th tvly- miss confirmed, 7th gitignore-wipe confirmed (unconditional rewrite at end of step 3 holds), AIza-in-sub...dule verified informational (no redaction needed), staged-blob scan defense confirmed working, cron-prompt dead-PAT escalation count now 6 consecutive runs
- `references/2026-07-23-session-notes.md` — cron prompt finally clean (dead-PAT escalation resolved); CJK + tvly- walker "misses" verified as **idempotent re-redaction** via `diff <(git show HEAD:"file") file` — new verification pattern; 8th gitignore-wipe reproduction; heavy-dir LEAK warnings under `hermes-agent/` (submodule) clarified as informational no-ops
- `references/2026-07-24-session-notes.md` — 9th CJK walker miss + 5th tvly- miss (both real, NOT idempotent — confirmed via `diff <(git show HEAD) file`); first-time clone today was the slow path (used `background=true` + `notify_on_complete=true` + `timeout=600`); new bug: inline `redact_prefix.py` snippet's `\b` regex stops at internal hyphens so on `tvly-dev-...` it matches `tvly-dev` only and silently no-ops; bundled `redact_secrets.py` single-file mode ALSO misses `tvly-` (verified today); canonical `redact_providers.py` working recipe now embedded in step 5 with multiple-prefix single-argv-pass design
- `references/2026-07-25-session-notes.md` — deterministic housekeeping preservation by excluding `.gitignore`/`.gitattributes` at the rsync boundary; successful independent length, all-file byte, staged-blob, and remote-hash verification sequence
...
- `scripts/redact_providers.py` — **multi-prefix truncation for prefixes the bundled walker doesn't cover** (tvly-, AIza-, glpat-, xoxb- etc.) — added 2026-07-24 to fix the silent-no-op bug in the 2026-07-19 inline `redact_prefix.py` snippet
...
- **"Redacted-at-source" files preserve secrets in committed `***` form (2026-07-14)** — Some files in `~/.hermes/` are committed with secrets already stripped at the source, e.g. `workspace/oriental-destiny/config.real.js` contains `apiKey: ***` (not the real value), `workspace/oriental-destiny/config.real.example.js` contains `apiKey: "AIzaSy-your-...here"` (29 chars, documentation placeholder). These files will appear "clean" in the length-gated scan but the **rsync source is the source of truth** — if the user ever un-redacts them on disk before a backup, the leak will land in the next commit. The walker doesn't need to touch these files (the literals are already short), but be aware: a redaction scanner that reports "all clean" can give false confidence when the *source* is what's broken. The redaction is one-directional (working-tree copy); the rsync source itself is never modified. This pairs with the "source itself is the leak" PAT-in-cron-prompt pattern.
- **Git submodules are invisible to the parent repo's commit (2026-07-19, verified again 2026-07-22, 2026-07-23)** — `workspace/oriental-destiny/` and `hermes-agent/` show up in `git ls-tree HEAD` as mode `160000` (submodule gitlinks). The parent's `git add -A` records ONLY the submodule commit SHA, not the submodule's internal files. Reproduced 2026-07-22: rsync brought over `workspace/oriental-destiny/config.real.js` containing a real `AIzaSy...` Google key (40+ chars), the step-5 byte scan flagged it, but `git show :workspace/oriental-destiny/config.real.js` correctly returned `fatal: Pathspec ... is in submodule 'workspace/oriental-destiny'` — confirming the parent commit cannot leak submodule contents. **Action:** when a scan hits a submodule path, do NOT redact it; verify with `git check-ignore` (which returns "is in submodule") or just `git ls-files --stage | awk '$1=="160000"'` to enumerate submodules, then leave the hit alone. Redacting it would modify rsync-source files (the submodule working tree is what rsync brought over, and modifying it doesn't help anyway since the parent doesn't commit it). **However:** (a) don't waste cycles investigating them; treat submodule-internal hits as informational, not blockers; (b) the SUBMODULE'S OWN integrity is tracked by its own commit SHA in the parent, so corruption or secret re-injection inside the submodule is captured by the parent's gitlink (`git submodule status` would show a dirty SHA) — let `git status` flag submodule status changes rather than the byte scanner. **Workflow:** identify submodules FIRST via `git ls-files --stage | awk '$1=="160000"{print $4}'`, then add them to the noise filter: `for sm in $SUBMODULES; do NOISE="$NOISE|$(echo $sm | sed 's|/|\\/|g')"; done`. Use this expanded NOISE in the step-5 grep pipe so submodule paths produce zero false positives. **Heavy-dir LEAK warnings under submodules are also informational** (2026-07-23): the post-rsync LEAK check will report `hermes-agent/venv` (607M), `hermes-agent/website` (27M), `hermes-agent/tests` (32M), `hermes-agent/ui-tui` (3.7M) as present in the working tree, but all four are inside the `hermes-agent` submodule and the parent doesn't track them. The 2026-07-09 heavy-dir-leak pitfall's worst-case numbers describe non-submodule scenarios and remain valid; submodule-internal versions are silent no-ops. Don't add submodule-internal heavy dirs to .gitignore — they're already invisible to the parent.
- **Tavily keys (`tvly-`) bypass the walker entirely (2026-07-19, reproduced 2026-07-21, 2026-07-22, 2026-07-23, **2026-07-24 — 5th confirmed run**)** — `redact_secrets.py` covers only `sk-` and `github_pat_`. A real Tavily key `tvly-dev-sAFTx-...` (58 chars) was found in three `MEMORY.md` files (`workspace/website/德米知识库/01-记忆系统/MEMORY.md`, `workspace/website_old/MEMORY.md`, `workspace/website_old/德米知识库/01-记忆系统/MEMORY.md`) after a "clean" walker pass — the walker scanned hundreds of files but left tvly- untouched. Real Tavily keys are 50+ chars and trivially length-gate-able; the walker simply lacks the regex. **The walker is NOT a complete provider list** — it covers what Hermes itself uses; user notes (`MEMORY.md`, skill docs, scratch files) document any provider the user has ever touched. **Fix:** step 5's grep scan block now includes a dedicated `tvly-` (40+ chars) line. When a future non-handled provider prefix surfaces, add a new length-gated grep block for it AND use the `redact_prefix.py` snippet (added to the LOOP GUARD section above) to truncate without re-running the batch walker. **Always run the prefix-aware `redact_prefix.py` ON EACH OF THE 3 KNOWN MEMORY.MD FILES** in addition to any newly-discovered path — the canonical three are stable across backups and the walker will miss them every run. **2026-07-23 update:** on the run that finally resolved the dead-PAT-in-cron-prompt escalation, all three tvly- walker "misses" turned out to be idempotent re-redactions (HEAD content already in placeholder form). The step-5 scan still flags them because the regex doesn't know about truncation; **first** verify `diff <(git show HEAD:"<path>") "<path>"` exits 0 before applying single-file redaction. If exit 0, no action needed.
...
- **PAT-in-cron-prompt Case A escalation RESOLVED on 2026-07-23 (after 6 consecutive reproductions 2026-07-13, 2026-07-14, 2026-07-16, 2026-07-17, 2026-07-21, 2026-07-22)** — The user finally rewrote the cron job to remove the embedded dead `github_pat_11B67EO2Y0...Q5` token. Today's cron prompt and `~/.hermes/cron/jobs.json` contain zero PAT-shaped strings. **Workflow consequence:** the lead-sentence "rewrite cron prompt" advisory is no longer required on every backup. If a future cron prompt regresses and re-embeds a PAT, this advisory comes back automatically — keep the recipe ready (lead template: "**ACTION NEEDED: Your cron job still embeds a dead PAT... Rewrite the cron prompt to use SSH auth**"). See `references/2026-07-23-session-notes.md` for the verification recipe.
...
- **`redact_secrets.py` single-file mode ALSO misses `tvly-` / non-`sk-` prefixes (2026-07-24)** — Confirmed today: running `python3 <skill_dir>/scripts/redact_secrets.py <backup_dir> workspace/website/中文目录/MEMORY.md` (single-file mode, the documented CJK-recovery path) printed `REDACTED: 0 PAT, 1 sk-` and reported success, but the working tree still contained the full 62-char `tvly-dev-sAFTx-2XjSFsXdR5Z...a7Si` on the next line. The bundled script's regex list (`sk`, `github_pat_`, `gh[pousr]_`) is hard-coded — single-file mode inherits the exact same coverage gap as batch mode. **Implication:** when the step-5 byte scan flags a `tvly-` / `glpat-` / `AIza` hit after running the bundled single-file script, ALWAYS chain-call the `/tmp/redact_providers.py` snippet (fixed 2026-07-24, includes `-` in body class) before re-scanning. The two scripts are complementary, not substitutable. **Future-proofing:** if a walker gap on a NEW prefix is discovered, patch BOTH the bundled `redact_secrets.py` AND this skill's inline `redact_providers.py` template in the same change — otherwise the next backup will repeat the gap.
