...
- **`bin/uv` and `bin/tirith` are 99MB / kilobytes compiled Python-binary bundles in `~/.hermes/bin/` containing literal sk- keys** — Reproduction 2026-07-11: `bin/uv` (compiled Rust binary, 99MB) contained a real sk-... key of 64+ characters when scanned with `strings(1)`. The walker redaction script truncates the literal regardless of whether it's a runtime credential or compiled library code (both are "match → truncate"). GitHub push succeeded with a warning ("File bin/uv is 99.65 MB; this is larger than GitHub's recommended maximum file size of 50.00 MB"), but a future run on a larger `bin/` could trip the 100MB hard limit. Options for future backups: (a) add `--exclude='bin/uv' --exclude='bin/tirith'` to rsync (loses restore capabilities for these tools), (b) keep them but ensure the walker truncates their literals (current behavior — works), (c) move them to Git LFS (extra setup). **Current best practice: keep them in rsync + let the walker truncate**; only switch to (a) if a future GitHub file-size hard-limit blocks push.
...
- **CJK-path files (Chinese / Japanese / Korean directory names) silently bypass the os.walk walker — EXPECTED, not rare (reproduced 2026-07-08, 2026-07-13, 2026-07-14)** — This is now a known recurring failure mode of the walker, not a one-off. In 2026-07-14 the walker reported 580 redactions across 163 files but step 5 length-gated scan still found 1 real 72-char `sk-kimi-i...NGGW` key in `./workspace/website/德米知识库/01-记忆系统/MEMORY.md`. The CJK-encoded path was skipped by `os.walk` for an unknown reason (possibly a stale `dirs[:]` filter, possibly a UnicodeDecodeError swallowed by `except (OSError, UnicodeError)`). **Workflow consequence:** the post-rsync step 5 length-gated grep scan is NOT optional — it is the only reliable verification. Plan for at least one CJK-path miss per backup and budget the single-file redaction round-trip accordingly. **Fix:** when step 5 finds a hit, re-run redaction DIRECTLY on the specific file path with `python3 <skill_dir>/scripts/redact_secrets.py <backup_dir> <file>`, NOT the batch walker. Loop until step 5 reports zero hits. The arg order is `<backup_dir> <file>` (NOT just `<file>` — passing only the file triggers BATCH mode and crashes with a misleading `NotADirectoryError`; see the single-file-mode arg-order pitfall below). See `references/cjk-walker-miss-recipes.md` for the full reproduction, three root-cause candidates, the inline fallback script, and the byte-level verification recipe.
...
- **PAT-in-cron-prompt Case A is now reproduced THREE times (2026-07-13, 2026-07-14, 2026-07-16) — the user has not yet rewritten the cron job** — Across three consecutive daily backup runs the cron prompt has embedded the same dead `github_pat_11B67EO2Y0...Q5` token, returning HTTP 401 on every auth check, and the cron prompt is still being delivered unchanged. This is no longer an edge case worth re-explaining — it is the expected state. **Workflow consequence:** when a cron prompt contains a `github_pat_` token, skip the advisory about "use SSH instead" in the body of the delivery and put it as the **lead sentence** of the response. The user's action item (rewrite the cron job) has not been acted on across 3 runs; a buried advisory won't get seen. Suggested lead template: "**ACTION NEEDED: Your cron job still embeds a dead PAT (`github...Q5`, HTTP 401 on three consecutive runs). Rewrite the cron prompt to use SSH auth — `git clone git@github.com:qzw-alt/demi.git` works without a token.** Backup pushed via SSH: ..." Then continue with the rest of the report. The PAT itself is dead so there's nothing to revoke (Case A); the only fix is rewriting the cron job source.
