...
> ⚠️ **Security pitfall — `workdir` and secret injection:** When `workdir`
> points to a directory whose `AGENTS.md` / `CLAUDE.md` / `MEMORY.md` files
> contain secrets (API keys, GitHub PATs, credentials), the scheduler injects
> those files into the prompt at runtime. This causes the prompt to be
> blocked by the `read_secrets` threat-pattern scanner before execution
> even begins. If the workdir contains files with sensitive data, either
> remove those files or omit the `workdir` setting entirely and pass all
> necessary context via the prompt or `skills` fields instead.
> See: `references/cron-workdir-secret-injection.md` for a full
...
`hermes --version` may exit 0 but output nothing / raise `ModuleNotFoundError: No module named 'hermes_cli'` on some installs (editable-deploy path mapping issue). Reliable alternatives:
```bash
pip index versions hermes-agent   # PyPI latest
pip show hermes-agent | grep Version   # installed package version
curl -s https://pypi.org/pypi/hermes-agent/json | python3 -c "import sys,json; print(json.load(sys.stdin)['info']['version'])"  # PyPI latest from API
```
