...
**Trust `git ls-remote --heads origin` over user-instructions or memory about branch names.** Per-site verified branches (2026-06-04):
...
```
Root cause: the remote URL has no token embedded (`https://github.com/qzw-alt/chinahospitalsguide.git`), and `~/.git-credentials` in the cron sandbox is masked (only `https://github...MbQ5@github.com` is visible — the agent cannot read the actual PAT). No `GH_TOKEN` env var, no `gh` CLI installed, `git credential fill` returns "credential missing host field".
