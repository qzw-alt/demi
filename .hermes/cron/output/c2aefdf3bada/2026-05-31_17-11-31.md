...
⚠️ **Git push blocked**: No GitHub credentials available (no `GITHUB_TOKEN`, no SSH keys, no `gh` CLI). The article is committed locally but cannot be pushed to the remote. You need to either:
- Add a `GITHUB_TOKEN` to `~/.hermes/.env`
- Configure SSH keys for GitHub
- Or push manually: `cd ~/.hermes/workspace/oriental-destiny && git push`

---

**Article Summary — Feng Shui Home Office (fate-2026-05-31.html)**

- **Title**: 7 Feng Shui Tips for Your Home Office That Actually Make a Difference
- **Body word count**: ~1,200 words
- **Humanizer score**: ~65–70 (passed; natural conversational tone, varied sentence rhythm, opinionated framing, removed AI patterns — no filler phrases, no copula avoidance, no signposting, no em-dash spam, no rule-of-three lists)
- **Humanizer patterns addressed**:
  - ✅ Removed "good news is", "you do not need to move walls" opener made conversational
  - ✅ Varied sentence length (short punchy + longer flowing)
  - ✅ Added personal observations ("It feels grounding even if you do not consciously notice it")
  - ✅ Removed rule-of-three ("focused, creative, and productive")
  - ✅ Replaced em dashes with commas/periods for smoother flow
  - ✅ Removed generic positive conclusion
  - ✅ No hedging, no vague attributions, no chatbots artifacts

**Published URL** (once pushed): `https://oriental-destiny.com/fate-2026-05-31.html`

**Files changed** (ready to push):
- `fate-2026-05-31.html` — new article (158 lines, 11,687 bytes)
- `sitemap.xml` — added article entry
